...
void DormantLightPanelLED( void);
...
static unsigned int FrontPanelLEDs[NO_OF_CUBES];
    ...
    switch ( CurrentState )
    {
         ...
         case Game_Running : 
            ...
            {  puts("cube correct in game");
               UpdateUsersAnswer(ThisEvent.EventParam);
			      if (True/*strcmp(usersanswer,computersanswer)==0*/){
...
void DormantLightPanelLED(void)
{
   ...
   FrontPanelLEDs[0] = B16(00000000,00001110) ;//Light up 0th LED
   FrontPanelLEDs[1] = B16(00000000,01110000) ;//Light up 1st LED
   FrontPanelLEDs[2] = B16(00000011,10000000) ;//Light up 2nd LED
   FrontPanelLEDs[3] = B16(00011100,00000000) ;//Light up 3rd LED
   FrontPanelLEDs[4] = B16(11100000,00000000) ;//Light up 4th LED
   
   /*FrontPanelLEDs[0] = RED+GREEN+BLUE ;//Light up 0th LED
   FrontPanelLEDs[1] = (RED+GREEN+BLUE)<<3 ;//Light up 1st LED
   FrontPanelLEDs[2] = (RED+GREEN+BLUE)<<6 ;//Light up 2nd LED
   FrontPanelLEDs[3] = (RED+GREEN+BLUE)<<9 ;//Light up 3rd LED
   FrontPanelLEDs[4] = (RED+GREEN+BLUE)<<12 ;//Light up 4th LED  */
   
   for(j = 0; j < NO_OF_CUBES; j++)
   {      
      for(i = 0; i < 16; i++) //Repeat 16 times
      {
         if ((FrontPanelLEDs[j] & B16(00000000,00000001)) == 0x1)//place FrontPanelLED onto the data output pin 
         ...
         FrontPanelLEDs[j] = FrontPanelLEDs[j]>>1; //Shift FrontPanelLED[i] 1 position to the right
      }
      PulseRCKU4U5();
...
void ClearAllLEDs(void)
{
   ...
   for (i = 0; i < NO_OF_CUBES; i++)
   {
     FrontPanelLEDs[i] = B16(00000000,00000000); //resets internal representation of Front Panel LEDs  
   }
...
//up and down in an alternating fashion for 2 complete loops. 
 void AlternatePillars(void)
 {
   ...
   startTime=EF_Timer_GetTime();//wait for 3000 milliseconds and therefore 3 sec
   while(EF_Timer_GetTime()<(startTime+3000));
