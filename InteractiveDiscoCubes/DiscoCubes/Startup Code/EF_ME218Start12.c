...
// add new temp location for storage of CCR during critical sections
// do it here to be sure that it is defined only once  jec 10/21/11
...
#pragma MESSAGE DISABLE C4000 /* condition always true in while(1) */
#pragma NO_FRAME
#pragma NO_ENTRY
#pragma NO_EXIT
...
/*set up the PLL to supply a 24MHz clock assuming an 8MHz xtal*/
        CLKSEL &= ~_S12_PLLSEL;         /*disengage PLL to system*/
        PLLCTL |= _S12_PLLON;           /*turn on PLL*/
        SYNR = 2;                       /*set PLL multiplier*/
        REFDV = 0;                      /*set PLL divider*/
__asm {                   /* a little delay before starting the test loop */
        nop
        nop
}
        while ((CRGFLG & _S12_LOCK) != _S12_LOCK)
                ;            /* wait for PLL to lock */
        CLKSEL |= _S12_PLLSEL;          /*engage PLL */
...
/* Clear IRQEN to make Port E1 available as an input*/
      ...
      Fini();
#endif
		  while(1);  /* hang if return from main() */
   } /* end loop forever */
}
